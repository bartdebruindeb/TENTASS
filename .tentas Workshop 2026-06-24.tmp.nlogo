...
to setup-social-innovators

 ; Assigns which agents are the social innovators (green) at the start of the simulation. Based on Centola (2018) you choose between Shotgun (assigning the
...
to consider-adopting-new-behavior
  ifelse module = "stochastic" [
    ask people [
      ifelse my-behavior = 0 [
        if random-float 1 < (count peers with [my-behavior-t-1 = 1] + k1) / (count peers + k2) [                   ; Where k1 < k2
          set my-behavior 1
          set color green
        ]
      ][
        if random-float 1 < (count peers with [my-behavior-t-1 = 0] + k1) / (count peers + k2) [
          set my-behavior 0
          set color blue
        ]
      ]
  ...
  ask people [
    if my-behavior = 0 [
      if my-threshold < (count peers with [my-behavior-t-1 = 1] + k1) / (count peers + k2) [
        set my-behavior 1
        set color green
...
seeding-strategy
seeding-strategy
"Snowball" "Shotgun" "Silver-Bullets"
1
...
"" ""
PENS
"default" 1.0 0 -13840069 true "" "plot count people with [my-behavior = 1]"
"pen-1" 1.0 0 -13345367 true "" "plot count people with [my-behavior = 0]"
...
@#$#@#$#@
<experiments>
  <experiment name="Stochastic" repetitions="400" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
  ...
  <experiment name="Threshold" repetitions="400" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>report-sim-ended</exitCondition>
    <metric>MSCe</metric>
